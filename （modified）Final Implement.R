...
# Step 1: 把 mortality_surface 中的日期列作为主时间索引
mortality_surface[, date := as.Date(date)]

# Step 2: 重新生成 mort_mat，并且设置日期为 Date 类型 rownames
mort_mat <- as.matrix(mortality_surface[, -1, with = FALSE])
rownames(mort_mat) <- as.character(mortality_surface$date)  # 先设为字符串
all_dates <- as.Date(rownames(mort_mat))                    # 再转换为 Date

# Step 3: 移除非法日期行（例如 0000-01-01 或 NA）
valid_indices <- which(!is.na(all_dates) & all_dates > as.Date("2000-01-01"))  # 加一个门槛
...
dim(mort_mat)           # 确保仍然是 109 x 13
length(train_dates)     # 应该是 87
length(test_dates)      # 应该是 22
range(train_dates)      # 应该是 "2015-01-01" 到 "2022-03-01"
range(test_dates)       # 应该是 "2022-04-01" 到 "2023-12-01"

# Split the cleaned mortality matrix into training and testing sets
